...
export class Animation {
	 ...
	 */
	public static ease(inORout: 'in' | 'out', direction: Direction, sel: string | HTMLElement | Array<string> | Array<HTMLElement>, duration: number = 750, delay: number = 0, endpoint: number = 100) {
		anime({
			targets: sel,
			translateY: direction === 'top' || direction === 'bottom' ? (inORout === 'in' ? [direction === 'top' ? endpoint : -endpoint, 0] : [0, endpoint]) : [0, 0],
			translateX: direction === 'left' || direction === 'right' ? (inORout === 'in' ? [direction === 'right' ? endpoint : -endpoint, 0] : [0, endpoint]) : [0, 0],
...
export function fixLoopSwiper(inst: Vue, backgroundEl: string) {
	inst.$Lazyload.$on('loaded', ({ el, src }) => {
		...
		let dup = parent?.parentNode?.querySelector(".swiper-slide-duplicate[data-swiper-slide-index='" + parent.getAttribute('data-swiper-slide-index') + "']");
